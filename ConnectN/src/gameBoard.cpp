...
void gameBoard::dropADisc(int col)
{
	...
	if(isMyTurn)
	{
		...
		cerr << "ADDING PIECE TO PLAYER " << firstEmptyCellRow << ","<<col<< "\n";
		cerr << "  Num pieces: "<<playerPieces.size()<<"\n";
		cerr << "  Num pieces: "<<opponentPieces.size()<<"\n";
	}
	else
	{
		cerr<<"opp turn"<<endl;
		...
		cerr << "ADDING PIECE TO OPPONENT " << firstEmptyCellRow << ","<<col<< "\n";
...
void gameBoard::printBoard()
{
	...
	cerr << "//BOARD INFO//\n";
	cerr << "  Num player pieces: "<<playerPieces.size()<<"\n";
	cerr << "  Num opponent pieces: "<<opponentPieces.size()<<"\n";


	for(vector<GamePiece>::const_iterator it = playerPieces.begin();
			it != playerPieces.end(); ++it)
	{
		GamePiece i = *it;
		cerr << "  PPiece[" << i.row << "," << i.col << "]\n";
	}
	...
	map<int,int> PCombos = comboCounter(PLAYER);
	cerr << "PCombosSize = "<< PCombos.size() << "\n";
	for(map<int, int>::const_iterator it = PCombos.begin();
			it != PCombos.end(); ++it)
	{
		cerr << "  PCombos["<<it->first<<"]="<<it->second<<"\n";
	}

	map<int,int> OCombos = comboCounter(OPPONENT);
	cerr << "OCombosSize = "<< OCombos.size() << "\n";
	for(map<int, int>::const_iterator it = OCombos.begin();
			it != OCombos.end(); ++it)
	{
		cerr << "  OCombos["<<it->first<<"]="<<it->second<<"\n";
	}

	cerr << "HVAL: " <<heuristicEvaluator()<<"\n";
